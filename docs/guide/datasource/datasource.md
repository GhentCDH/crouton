...
what the [CLI / codegen](../cli/cli.md) needs to introspect and generate.
...
These are read only by the `crouton` CLI / codegen engine; the runtime ignores them. They make multiple datasources
possible by giving each its own schema and generated outputs.
...
Resources without a `database` field use the data source marked `default: true`. A single datasource can keep flat paths
(e.g. `prisma/schema.prisma`, `generated/types`) — the per-`<name>` layout above only becomes necessary once a second
datasource is added.

The CLI **discovers** datasources by scanning the data-sources directory (set in [`crouton.json`](../cli/cli.md)); they
are not listed anywhere else.
