...
describe('resolveDatasource', () => {
  it('returns the single datasource', () => {
    expect(resolveDatasource(base)).toMatchObject({
      name: 'docsdb',
      urlEnv: 'DATABASE_APP_URL',
    });
  });
  it('honors the explicit name', () => {
    const cfg = {
      ...base,
      datasources: {
        a: { prismaSchema: 'a.prisma' },
        b: { prismaSchema: 'b.prisma', default: true },
      },
    };
...
describe('makeSchemaExportName', () => {
  it('applies the template', () => {
    expect(makeSchemaExportName(base)('Language')).toBe(
      'LanguageWithRelationsSchema',
    );
    expect(
      makeSchemaExportName({ ...base, schemaExportName: '{Model}Model' })(
        'Text',
      ),
    ).toBe('TextModel');
  });
...
describe('validateConfig', () => {
  it('rejects missing required fields and double defaults', () => {
    expect(() => validateConfig({ ...base, resourcesDir: '' })).toThrow(
      /resourcesDir/,
    );
    expect(() => validateConfig({ ...base, datasources: {} })).toThrow(
    ...
    expect(() =>
      validateConfig({
        ...base,
        datasources: {
          a: { prismaSchema: 'a', default: true },
          b: { prismaSchema: 'b', default: true },
        },
      }),
...
describe('loadConfig + scaffoldConfigFromProject', () => {
  it('loads a crouton.json by walking up from a subdir', async () => {
  ...
  it('scaffolds by detecting prisma url env, datasources, and the types import', async () => {
    ...
    await writeFile(
      ...
      'export default { datasource: { url: env("DATABASE_APP_URL") } };',
    );
    const dsDir = join(
      root,
      'apps',
    ...
    await writeFile(
      ...
      JSON.stringify({ name: 'docsdb', default: true }),
    );
    const resDir = join(
      root,
      'apps',
    ...
    expect(config.datasources.docsdb).toMatchObject({
      urlEnv: 'DATABASE_APP_URL',
      default: true,
    });
