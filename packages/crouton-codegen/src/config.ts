...
const CONFIG_FILES = ['crouton.config.json', 'crouton.config.mjs', 'crouton.config.js'];
...
export const validateConfig = (config: CroutonConfig, path = '<config>'): void => {
  ...
  if (names.length === 0) throw new Error(`${path}: at least one datasource is required.`);
...
export const resolveDatasource = (
  ...
  let chosen = name;
  if (!chosen) {
    chosen = names.find((n) => config.datasources[n].default) ?? (names.length === 1 ? names[0] : undefined);
  }
  if (!chosen) {
    throw new Error(
  ...
  if (!ds) throw new Error(`Unknown datasource "${chosen}". Available: ${names.join(', ')}.`);
...
export const scaffoldConfigFromProject = async (root: string): Promise<ScaffoldResult> => {
  ...
  const prismaSchema =
    (await firstExisting(root, ['prisma/schema.prisma', 'schema.prisma'])) ?? 'prisma/schema.prisma';

  let urlEnv: string | undefined;
  const prismaConfig = await firstExisting(root, ['prisma.config.ts', 'prisma.config.js', 'prisma.config.mjs']);
  if (prismaConfig) {
    const text = await readFile(join(root, prismaConfig), 'utf-8');
    urlEnv = /env\(\s*["']([^"']+)["']\s*\)/.exec(text)?.[1];
  }
  if (!urlEnv) notes.push('Could not detect the DB URL env var; set "urlEnv" manually.');
  ...
  if (dataSourcesDir) {
    ...
    for (const e of entries) {
      ...
      const dsJson = JSON.parse(await readFile(dsJsonPath, 'utf-8')) as { name?: string; default?: boolean };
      const name = dsJson.name ?? e.name;
      datasources[name] = { prismaSchema, urlEnv, ...(dsJson.default ? { default: true } : {}) };
    }
  }
  ...
  if (!generatedTypesImport) {
    ...
    notes.push('Could not detect the generated-types import; set "generatedTypesImport" manually.');
  }
