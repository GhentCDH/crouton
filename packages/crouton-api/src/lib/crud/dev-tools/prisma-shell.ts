/**
 * Thin wrappers around the project's Prisma CLI plus a git-dirty check for
 * the destructive `db pull` step.
 *
 * This deliberately duplicates `crouton-cli/src/update/prisma.ts` rather than
 * importing it: `crouton-cli` is a `bin`-only package built around `@clack/prompts`
 * for an interactive terminal session, not meant to be depended on as a
 * library, and its `exports` map doesn't expose this module. The functions
 * here are plain `node:child_process`/`node:fs` wrappers with no CLI-specific
 * dependencies, so keeping a small parallel copy is simpler than reshaping
 * crouton-cli into an importable package for it.
 */
...
export const prismaDbPull = async (
  cwd: string,
  prismaConfig: string,
): Promise<PrismaRunResult> => {
  const { code, stdout, stderr } = await run(
    'npx',
    ['prisma', 'db', 'pull', '--config', prismaConfig],
    cwd,
  );
...
export const prismaCaseFormat = async (
  cwd: string,
  schemaPath: string,
): Promise<PrismaRunResult> => {
  const { code, stdout, stderr } = await run(
    'npx',
    ['prisma-case-format', '--file', schemaPath],
    cwd,
  );
  return { ok: code === 0, output: `${stdout}\n${stderr}`.trim() };
};
...
export const prismaGenerate = async (
  cwd: string,
  prismaConfig: string,
): Promise<PrismaRunResult> => {
  const { code, stdout, stderr } = await run(
    'npx',
    ['prisma', 'generate', '--config', prismaConfig],
    cwd,
  );
  return { ok: code === 0, output: `${stdout}\n${stderr}`.trim() };
};
