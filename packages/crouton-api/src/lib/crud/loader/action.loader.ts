...
export const loadActions = async (
  ...
  for (const action of jsonActions) {
    ...
    if (!file) {
      console.warn(
        `[actions] Procedure file not found for "${action.id}" in ${basePath}/actions/`,
      );
      continue;
    }
    const mod =
      await importDefault<(prisma: any, id: string | number) => Promise<any>>(
        file,
      );
    if (!mod) {
...
export const loadTableActions = async (
  ...
  for (const action of jsonActions) {
    ...
    if (!file) {
      console.warn(
        `[tableActions] Procedure file not found for "${action.id}" in ${basePath}/actions/`,
      );
      continue;
    }
