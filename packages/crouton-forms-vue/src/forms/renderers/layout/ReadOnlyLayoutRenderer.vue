<template>
  <div v-if="isLayout" :class="getLayout">
    <div
      ...
      :key="i"
      :class="[COLSPAN_READ_ONLY[(child as any).options?.colspan ?? 12], ROWSPAN[(child as any).options?.rowspan ?? 0]]"
    >
...
import { COLSPAN_READ_ONLY, ROWSPAN } from './colspan';
...
const LAYOUT: Record<string, string> = {
  ...
  GridLayout: 'grid grid-cols-1 gap-x-3 grid-cols-10 lg:grid-cols-8 grid-flow-row-dense',
